...
import { SSHConfig, parse, LineType, type Line } from '../../dist/ssh-config.js'
import { readFixture } from '../helpers.cjs'

const { COMMENT, DIRECTIVE } = SSHConfig
...
describe('parse', function() {
  ...
  it('.parse simple config', async function() {
    const config = parse(await readFixture('config'))
  ...
  it('.parse unquoted values that contain double quotes', function() {
    ...
    assert.deepEqual(config[0].value.map(({ val }: { val: string }) => val), [
      'ssh',
      '-W',
      '%h:%p',
  ...
  it('.parse ProxyCommand with multiple args, some quoted', function() {
    ...
    assert.deepEqual(config[0].config[0].value.map(({ val }: { val: string }) => val), ['C:\\foo bar\\baz.exe', 'arg', 'arg', 'arg'])
  })
  ...
  it('.parse standalone match criteria', function() {
    ...
    `)
    const match = config.find((line: Line) => line.type === DIRECTIVE && line.param === 'Match')
    assert.ok(match)
  ...
  it('.parse match with comments', function() {
    ...
    assert.deepEqual(match.config.find((entry: Line) => 'param' in entry && entry.param === 'Include'), {
      after: '',
      before: '        ',
      param: 'Include',
      separator: ' ',
