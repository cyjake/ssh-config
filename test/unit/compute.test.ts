...
describe('compute', function() {
  ...
  it('.compute By Host with canonical domains', async function() {
    const config = SSHConfig.parse(`
      ...
      Host www
        ...
        CanonicalDomains example.notfound example.com
    `)
  ...
  it('.compute by Match host uses HostName', async function() {
    const config = SSHConfig.parse(`
      ...
      Match host *.com
        ProxyJump proxy.com
    `)
    const result: Record<string, string | string[]> = config.compute({ Host: 'tahoe' })
    assert.ok(result)
    assert.equal(result.HostName, 'tahoe.com')
  ...
  it('.compute should fallback to process.env.USERNAME on Windows', async () => {
    const mock = sinon.mock(os)
    mock.expects('userInfo').throws(new Error('user has no username or homedir'))
    const originEnv: Record<string, string> = {}
    for (const [key, value] of Object.entries({ USER: undefined, USERNAME: 'test' })) {
      if (process.env[key] != null) {
        originEnv[key] = process.env[key]
  ...
  it('.compute should preserve separators in multi-value directives', async () => {
    ...
    const result: Record<string, string | string[]> = config.compute({ Host: 'YYYY' })
